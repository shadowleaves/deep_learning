...
class DataIter(mx.io.DataIter):
    def __init__(self, n_samples, batch_size, seq_len, n_inputs,
                 num_label, init_states, shuffle=False):
        ...
        self.X, self.T = create_dataset(n_samples, seq_len)
        # self.data = X
        # self.label = T
    ...
    def iter_next(self):
        return self.cur < self.n_samples

    def getindex(self):
        return self.cur / self.batch_size

    def getpad(self):
        return self.batch_size - self.n_samples % self.batch_size
    ...
    def get_batch(self):
        start = self.cur
        end = self.cur + min(self.batch_size, self.n_samples)

        self.data = mx.nd.array(self.X[self.index][start:end])
        self.label = mx.nd.array(self.T[self.index][start:end])
...
if __name__ == '__main__':
    ...
    if False:
        ...
        # for batch in enumerate(data_train):
        #     import pdb
        #     pdb.set_trace()
        ...
        for epoch in xrange(n_epochs):
            data_train.reset()
            print 'epoch %d' % epoch
            # for step in xrange(nb_train / batch_size):
