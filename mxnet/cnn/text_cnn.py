...
CNNModel = namedtuple(
    "CNNModel", ['cnn_exec', 'symbol', 'data', 'label', 'param_blocks'])
...
def make_text_cnn(sentence_size, num_embed, batch_size, vocab_size,
    ...
    if not with_embedding:
        embed_layer = mx.sym.Embedding(
            data=input_x, input_dim=vocab_size,
            output_dim=num_embed, name='vocab_embed')
        conv_input = mx.sym.Reshape(data=embed_layer, target_shape=(
            batch_size, 1, sentence_size, num_embed))
    else:
        conv_input = input_x
    ...
    for i, filter_size in enumerate(filter_list):
        convi = mx.sym.Convolution(data=conv_input, kernel=(
            filter_size, num_embed), num_filter=num_filter)
        relui = mx.sym.Activation(data=convi, act_type='relu')
        pooli = mx.sym.Pooling(data=relui, pool_type='max', kernel=(
    ...
    h_pool = mx.sym.Reshape(
        data=concat, target_shape=(batch_size, total_filters))
    ...
    fc = mx.sym.FullyConnected(
        data=h_drop, weight=cls_weight, bias=cls_bias, num_hidden=num_label)
...
def setup_cnn_model(ctx, batch_size, sentence_size, num_embed, vocab_size,
    ...
    cnn_exec = cnn.bind(ctx=ctx, args=arg_arrays,
...
def train_cnn(model, X_train_batch, y_train_batch, X_dev_batch, y_dev_batch, batch_size,
    ...
    for iteration in range(epoch):
        ...
        for begin in range(0, X_train_batch.shape[0], batch_size):
            ...
            num_correct += sum(batchY ==
                               np.argmax(m.cnn_exec.outputs[0].asnumpy(), axis=1))
        ...
        if (iteration + 1) % 10 == 0:
            ...
            save_dict = {('arg:%s' % k): v for k,
                         v in m.cnn_exec.arg_dict.items()}
            save_dict.update(
                {('aux:%s' % k): v for k, v in m.cnn_exec.aux_dict.items()})
...
def main():
    ...
    x_dev = np.reshape(
        x_dev, (x_dev.shape[0], 1, x_dev.shape[1], x_dev.shape[2]))
    ...
    cnn_model = setup_cnn_model(
        mx.cpu(0), batch_size, sentence_size, num_embed, dropout=0.5)
...
def train_without_pretrained_embedding():
    ...
    cnn_model = setup_cnn_model(mx.cpu(0), batch_size, sentence_size,
                                num_embed, vocab_size, dropout=0.5, with_embedding=False)
    train_cnn(cnn_model, x_train, y_train, x_dev, y_dev, batch_size)
